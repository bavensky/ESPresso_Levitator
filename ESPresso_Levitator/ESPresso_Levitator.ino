...
void setup()
{
  ...
  ticker_PID.attach_ms(5, PID);
  ticker_DEGUG.attach_ms(100, Debug);
...
//  digitalWrite(LED, 0);
  ...
  /* if fix referent */
  kp = 200.0f;  //130 150 180* 185 200 300
  ki = 5.0f;  // 3 5* 8
  kd = 30.0f; // 30* 50
  ...
  output = kp * error + ki * error_sum + kd * error_dot + 200; // offser g + 200 + 222 + 252 + 500

  if (output < 0)   output = 0;
  if (output > 1023) output = 1023;

  if (state < 5.5f || state > 8.0f)
  {
    output = 0;
    error_sum = 0;
    error_dot = 0;
